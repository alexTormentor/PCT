...
int main(int argc, char** argv) {
    ...
    MPI_Request requests[nnodes];
    MPI_Status statuses[nnodes];
    ...
    for (int i = 0; i < nnodes; i++) {
        if (i == rank) {
            for (int j = 0; j < nnodes; j++) {
                if (i != j) {
                    int dest;
                    MPI_Graph_neighbors_count(comm_graph, i, &dest);
                    MPI_Send(&A[0], N * N, MPI_INT, j, 0, comm_graph);
                }
            }
        }
        else {
            ...
            MPI_Recv(&A[0], N * N, MPI_INT, i, 0, comm_graph, MPI_STATUS_IGNORE);
        }
    }
